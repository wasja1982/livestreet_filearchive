...
{if !$oUserCurrent && $bOnlyUsers}
	<a href="{router page='registration'}" style="display:block;">{$aLang.plugin.filearchive.topic_file_access_denied}</a>
{else}
	...
	<a href="{$oTopic->getDownloadUrl()}" style="display:block;" title="{$aLang.plugin.filearchive.topic_file_downloads}: {$oTopic->getFileDownloads()}">{$aLang.plugin.filearchive.topic_file_download} "{$oTopic->getFileName()}" {($oTopic->getFileSize() / 1024)|string_format:$aLang.plugin.filearchive.topic_file_size}</a>
    {else}
